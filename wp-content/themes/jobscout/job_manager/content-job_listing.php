...
<article <?php job_listing_class(); ?> data-longitude="<?php echo esc_attr($post->geolocation_lat); ?>" data-latitude="<?php echo esc_attr($post->geolocation_long); ?>">
	...
	<div class="job-title-wrap section-top-job">
		...
		<?php if ($company_name) { ?>
			<div class="time-tob-job">
				Created :
				<time datetime="<?php printf(__('%1$s', 'raratheme-companion'), get_post_time('M d, Y')); ?>"><?php printf(__('%1$s', 'raratheme-companion'), get_post_time('M d, Y')); ?></time></a>
			</div>
		<?php } ?>
	...
	<div class="job_description custom-limit-lines">
		<?php
		$jobDescription = wpjm_the_job_description(); // Lấy nội dung job description

		$limitedDescription = wp_trim_words($jobDescription, 20, '...'); 

		echo $limitedDescription; // Hiển thị nội dung đã được giới hạn
		?>
	</div>
